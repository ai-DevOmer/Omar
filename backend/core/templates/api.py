...
class TemplateResponse(BaseModel):
    ...
    categories: List[str]
    is_public: bool
    is_omar-ai_team: Optional[bool] = False
...
@router.get("/omar-ai-all", response_model=MarketplaceTemplatesResponse)
async def get_all_omar-ai_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all OMAR AI templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_omar-ai_team: Optional[bool] = Query(None, description="Filter for OMAR AI team templates"),
    ...
    try:
        ...
        filters = MarketplaceFilters(
            search=search,
            tags=tags_list,
            is_omar-ai_team=is_omar-ai_team,

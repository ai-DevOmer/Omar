...
from omar-ai.utils import print_stream
...
async def main():
    ...
    omar-ai_client = omar-ai.OMAR AI(
    ...
    if not agent_id:
        agent = await omar-ai_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await omar-ai_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await omar-ai_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await omar-ai_client.Thread.get(thread_id)

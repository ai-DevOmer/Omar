...
const getDefaultModel = (accessibleModels: ModelOption[]): string => {
  ...
  // omar-ai/basic should be first for free users since power is not accessible
  const basicModel = accessibleModels.find(m => m.id === 'omar-ai/basic');
  if (basicModel) return basicModel.id;

  const powerModel = accessibleModels.find(m => m.id === 'omar-ai/power');
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      const powerModel = availableModels.find(m => m.id === 'omar-ai/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to omar-ai/power');
        setSelectedModel('omar-ai/power');
      }
    }

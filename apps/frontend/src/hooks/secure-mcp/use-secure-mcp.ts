...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      ...
      if (params?.is_omar-ai_team !== undefined) searchParams.set('is_omar-ai_team', params.is_omar-ai_team.toString());
...
export function useOMAR AITeamTemplates(options?: { enabled?: boolean }) {
  return useQuery({
    queryKey: ['secure-mcp', 'omar-ai-templates-all'],
    queryFn: async (): Promise<MarketplaceTemplatesResponse> => {
      const supabase = createClient();
      const { data: { session } } = await supabase.auth.getSession();

      if (!session) {
        throw new Error('You must be logged in to view OMAR AI templates');
      }

      const response = await fetch(`${API_URL}/templates/omar-ai-all`, {
        headers: {
          'Authorization': `Bearer ${session.access_token}`,
        },
      });

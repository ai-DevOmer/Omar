...
import { ToolCallInput } from '@/components/thread/omar-ai-computer';
...
import { useOMAR AIComputerStore, useIsSidePanelOpen, useSetIsSidePanelOpen } from '@/stores/omar-ai-computer-store';
...
export function useThreadToolCalls(
  ...
  const navigateToToolCall = useOMAR AIComputerStore((state) => state.navigateToToolCall);
  ...
  const handleToolClick = useCallback((clickedAssistantMessageId: string | null, clickedToolName: string, toolCallId?: string) => {
    ...
    const navigateToIndex = (index: number) => {
      ...
      setIsSidePanelOpen(true);
      // Use store action to ensure OMAR AIComputer switches to tools view

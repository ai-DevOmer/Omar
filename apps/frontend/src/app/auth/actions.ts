...
export async function signIn(prevState: any, formData: FormData) {
  ...
  // For desktop app, use custom protocol (omar-ai://auth/callback) - same as mobile
  // For web, use standard origin (https://omar-ai.com/auth/callback)
  let emailRedirectTo: string;
  if (isDesktopApp && origin.startsWith('omar-ai://')) {
    // Match mobile implementation - simple protocol URL with optional terms_accepted
    const params = new URLSearchParams();
    if (acceptedTerms) {
      params.set('terms_accepted', 'true');
    }
    emailRedirectTo = `omar-ai://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
  } else {

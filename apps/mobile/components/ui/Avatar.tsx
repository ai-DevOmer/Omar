...
interface AvatarProps extends ViewProps {
  ...
  /** Use OMAR AI symbol instead of icon (for SUNA agent) */
  useOMAR AISymbol?: boolean;
 ...
 * - OMAR AI symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for OMAR AI symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // OMAR AI symbol always uses solid black bg with white icon
    if (useOMAR AISymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useOMAR AISymbol ? 0 : (showBorder ? 1.5 : 0),
